...
export class PayoutService {
  ...
  static async requestPayout({ driverId, amount, payoutMethodId }) {
  ...
  static async createPayoutBatch(adminId, requestIds, notes = null) {
    ...
    return await withTransaction(async (client) => {
      ...
      for (const request of requests) {
        ...
        if (request.status !== "approved") {
          throw new AppError("PAYOUT_NOT_APPROVED", 400);
        }
      ...
      for (const request of requests) {
        ...
        const ledgerEntry = await LedgerService.processPayout({
          driverId: request.driver_id,
          amount: parseFloat(request.amount),
          payoutRequestId: request.id,
          client
        ...
        await PayoutRequest.updateOne(
          { id: request.id },
          { settlement_ledger_entry: ledgerEntry.id },
          client
        );
  ...
  static async processPayoutAttempt({
    ...
    return await withTransaction(async (client) => {
      ...
      if (success) {
        ...
        // Reverse the ledger entry (payout_holding → driver_wallet)
        await LedgerService.reverseEntry({
          originalEntryId: request.settlement_ledger_entry,
        ...
        await PayoutRequest.updateOne(
          { id: payoutRequestId },
          {
            ...
            failure_reason: failureReason,
            retry_count: request.retry_count + 1,
            next_retry_at: new Date(Date.now() + 86400000)
          },
          client
        );
